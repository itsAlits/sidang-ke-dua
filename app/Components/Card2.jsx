  ...
  return (
    <div className="card h-full relative">
      <div className="img-card overflow-hidden h-64 rounded-xl relative">
        <img src={props.img} alt={props.judul} className="w-full h-full object-cover" />
      ...
      <div className="content-card mt-6">
        <div className="flex justify-between items-start mb-2">
          ...
          <span className="badge badge-primary">{formatCategory(props.kategori)}</span>
        </div>
        ...
        {/* SAW Score (Only shown if sawScore is provided) */}
        {props.sawScore !== null && (
          <div className="mt-2 mb-2">
            <div className="flex justify-between text-xs text-gray-600 mb-1">
              <span>Skor Rekomendasi</span>
              <span className="font-medium">{props.sawScore.toFixed(4)}</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-1.5">
              <div 
                className="bg-primary h-1.5 rounded-full" 
                style={{ width: `${Math.min(props.sawScore * 100, 100)}%` }}
              ></div>
            </div>
          </div>
        )}
